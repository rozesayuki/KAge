...
class HalfModalViewController: UIViewController,UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    //誕生日を算出するメソッド
    @objc func birthdayButton() -> Int{
        let birthday = DateComponents(year: yearRow,month: monthRow,day: dayRow)
        var age: Int
        let thisYearBirthday = DateComponents(year: 2022,month: Int(monthPicker[monthRow])!,day: Int(dayPicker[dayRow])!)
    ...
    override func viewDidLoad() {
        ...
        button.addTarget(self, action: #selector(HalfModalViewController.birthdayButton()), for: .touchUpInside)
